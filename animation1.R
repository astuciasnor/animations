...
ggsave("middle_east_refugees_map.png", 
       plot = middle_east_refugees_map + facet_wrap(vars(year)), 
       width = 12, height = 8, units = "in", dpi = 300)



# Definir fps como um fator de 100
library(magick)
library(gganimate)


animation <- middle_east_refugees_map +
  labs(title = "Refugees as a percentage of total population in {current_frame}") +
  transition_manual(year)

animate(animation, nframes = 24, fps = 1)
